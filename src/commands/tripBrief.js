    ...
    try {
      ...
      if (Array.isArray(hotelsArray) && hotelsArray.length > 0) {
        ...
        hotelsArray.slice(0, 5).forEach((hotel, index) => {
          const safeName = encodeURIComponent((hotel.name ?? "hotel").substring(0, 50));
      ...
      for (let i = 1; i < messages.length; i++) {
        ...
        for (let j = 0; j < chunks.length; j++) {
          ...
          // Attach hotel buttons to the final chunk of the hotels section
          // Keep this index aligned with buildTripMessages() order
